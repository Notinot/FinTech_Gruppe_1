...
app.post('/invite-event', authenticateToken, async (req, res) => {
  try {
    ...
    if (checkForSpam.length > 0) {
     ...
     //check if users have each other blocked
     const [blocked] = await db.query('SELECT * FROM Friendship WHERE (requester_id = ? AND addressee_id = ?) AND status = "blocked" OR (requester_id = ? AND addressee_id = ?)', [senderId, recipientId, recipientId, senderId]);
     if (blocked.length > 0) {
